...
export default function ShopPage() {
  ...
  const [activeTab,    setActiveTab]    = useState('TRUFFLES')
  ...
  useEffect(() => {
    setLoading(true)
    const collectionHandle = activeTab === 'TRUFFLES' ? 'truffles' : 'apparel'
    getProductsByCollection(collectionHandle)
      .then(raw => {
        ...
        if (productHandle) {
          const found = normalized.find(p => p.handle === productHandle)
    ...
    >
      ...
      {shopStep === 'products' && (
        <>
          ...
          >
            {['TRUFFLES', 'APPAREL'].map(tab => {
              const active = activeTab === tab
              return (
                <button key={tab} className="shop-filter-btn" onClick={() => setActiveTab(tab)}
                  style={{
